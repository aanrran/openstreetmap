...
bool Compare(RouteModel::Node const *a, RouteModel::Node const *b) {
  int f1 = a->g_value + a->h_value; // f1 = ga + ha
  int f2 = b->g_value + b->h_value;// f2 = gb + hb
  return f1 > f2; 
}

RouteModel::Node *RoutePlanner::NextNode() {
  auto *sorted_openlist = &(this->open_list);
  std::sort(sorted_openlist->begin(), sorted_openlist->end(), Compare);
  RouteModel::Node *next_node = (*sorted_openlist).back();
  (*sorted_openlist).pop_back();
  return next_node;
}
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // TODO: Implement your solution here.
      ...
      // add the nodes, distance until reach the start point(whose parent is nullptr)
